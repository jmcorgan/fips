...
/// A peer that is eligible in every respect except that it does not advertise
/// the Full node profile, so it carries no transit bloom filter and must not
/// be selected as a next hop.
...
#[test]
fn candidate_selection_excludes_non_full_peers() {
    let dest = make_node_addr(0x50);
    let root = 0x00;
    let my_coords = make_coords(&[0x10, root]);
    let dest_coords = make_coords(&[0x50, root]);
    ...
    let rv = MockRoutingView {
        peers: vec![non_full.clone(), full.clone()],
        ..MockRoutingView::new(false)
    };
    assert_eq!(
        choose_candidate(&rv, &dest, &dest_coords, &my_coords),
        Some(make_node_addr(0x70)),
        "the cheaper non-Full peer must not win"
    );

    let only_non_full = MockRoutingView {
        peers: vec![non_full],
        ..MockRoutingView::new(false)
    };
    assert_eq!(
        choose_candidate(&only_non_full, &dest, &dest_coords, &my_coords),
        None,
        "with no Full candidate there is no bloom next hop at all"
    );
}
