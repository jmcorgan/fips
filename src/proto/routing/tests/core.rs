...
fn routing_candidates_filters_by_may_reach_and_snapshots() {
    ...
    let non_full_reacher = make_node_addr(0x62);
    ...
    let rv = MockRoutingView {
        peers: vec![
            ...
            MockPeer {
                // Bloom may_reach the dest, but not a Full-profile peer —
                // narrowed out by the Full filter.
                addr: non_full_reacher,
                reach: vec![dest],
                can_send: true,
                link_cost: 0.5,
                coords: None,
    ...
    assert_eq!(
        candidates.len(),
        1,
        "only Full peers whose bloom may_reach the dest survive assembly"
    );
    let c = &candidates[0];
