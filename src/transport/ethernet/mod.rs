...
impl EthernetTransport {
    ...
    pub fn new(
        ...
        Self {
            ...
            // 1496 = the common 1500-byte interface MTU minus the 4-byte frame
            // header; replaced by the real value at the first bind.
            binding: Arc::new(Binding::new(1496)),
    ...
    pub async fn start_async(&mut self) -> Result<(), TransportError> {
        ...
        let initial_unpublished: Option<bool> = match bind_and_spawn(&ctx).await {
            Ok(()) => publish_presence(&ctx, true),
...
fn bind_now(ctx: &Arc<BinderContext>) -> Result<(), TransportError> {
    ...
    if ctx.config.announce() {
        ...
        tasks.push(tokio::spawn(async move {
            beacon_sender_loop(socket, interval_secs, stats, transport_id).await;
        }));
    }
...
/// Publish a presence edge to node health. Returns the edge health is still
/// owed: `None` when it went out, `Some(present)` when the channel refused it
/// and the caller must retry it on the next tick.
///
/// **The return is the caller's pending slot, not a status.** Health is a
/// level, so an edge that reaches the consumer supersedes whatever earlier
/// edge was still waiting to be retried, and writing this result over the
/// pending slot is what makes that true. Reporting bare success and leaving
/// the slot to the caller let a landed edge sit behind a stale one: a `false`
/// republished over a transport that had since bound, or — the worse
/// direction — a `true` republished for an interface that had since detached.
/// The consumer does not dedup (`node::dataplane::rx_loop`), so that second
/// one leaves node health reading present for an interface that is gone, and
/// no reap running for the peers that can no longer be reached.
///
...
#[must_use]
fn publish_presence(ctx: &Arc<BinderContext>, present: bool) -> Option<bool> {
    ...
    match tx.try_send(TransportPresence {
        ...
        Err(tokio::sync::mpsc::error::TrySendError::Closed(_)) => {
            // Nobody is listening and nobody will be. Owing nothing stops the
            // caller retrying an edge that can never land.
            None
        }
        Err(tokio::sync::mpsc::error::TrySendError::Full(_)) => Some(present),
    }
}
...
async fn binder_loop(ctx: Arc<BinderContext>, initial_unpublished: Option<bool>) {
    ...
    // edge is superseded rather than queued. Supersession is `publish_presence`'s
    // job and not this loop's: every publish below writes its return over this
    // slot, so one that lands clears whatever was waiting in the same
    // statement. Seeded with the start edge if that one was refused.
    ...
    loop {
        ...
        if let Some(pending) = unpublished {
            unpublished = publish_presence(&ctx, pending);
        }
        ...
        if ctx.presence.is_present() {
            ...
            if detach.is_none() {
                ...
                if churn.stabilized(Instant::now()) {
                    ...
                    unpublished = publish_presence(&ctx, true);
                }
                continue;
            }
            ...
            // Only when an edge is actually owed. `retract` is
            // `mem::take(&mut announced)`, so it is false exactly when this
            // binding was never announced as present — there is then nothing
            // for a `false` to supersede, and settling the slot here would
            // drop a refused edge that is still owed rather than clear a
            // stale one.
            if outcome.retract {
                unpublished = publish_presence(&ctx, false);
            }
                ...
                match frame_type {
                    ...
                    FRAME_TYPE_BEACON => {
                        ...
                        if listen_enabled && parse_beacon(&buf[..len]) {
                            ...
                            // This line's beacon carries no pubkey, so there is
                            // no hint to pass through.
                            if !neighbor_buffer.add_peer(src_mac) {
...
mod tests {
    ...
    #[tokio::test]
    async fn an_edge_that_lands_supersedes_the_one_the_channel_refused() {
        // The pending slot holds the single edge health is owed, not a queue,
        // so a publish that lands has to clear it whatever value it was
        // holding: health is a level, and the value that just reached the
        // consumer is the current one.
        //
        // Both directions are asserted, because a fix aimed at one leaves the
        // other standing. A stale `false` left pending is republished over a
        // transport that has since bound. A stale `true` left pending is
        // republished for an interface that has since detached, and the
        // consumer does not dedup (`node::dataplane::rx_loop`), so node health
        // reads present for an interface that is gone and no reap runs for the
        // peers that can no longer be reached.
        let (eth, _rx) = absent_transport(false);
        let (tx, mut presence_rx) = tokio::sync::mpsc::channel(1);
        let ctx = Arc::new(BinderContext {
            shutdown: Arc::clone(&eth.shutdown),
            transport_id: eth.transport_id,
            ...
            binding: eth.binding.clone(),
            presence: eth.presence.clone(),
            presence_tx: Some(tx.clone()),
        });

        // Occupy the only slot, so the next edge is refused on its way out.
        tx.try_send(TransportPresence {
            transport_id: TransportId::new(99),
            present: true,
            health_relevant: true,
        ...
        // A refused edge is owed, and a refusal must not clear the slot —
        // that is what makes the retry happen at all. A fix that settles
        // unconditionally fails here rather than passing the two below.
        assert_eq!(
            publish_presence(&ctx, true),
            Some(true),
            "a refused edge stays owed"
        );

        // The consumer catches up, and a detach lands. It supersedes the
        // `true` that never went out.
        presence_rx.recv().await.expect("the squatter drains");
        assert_eq!(
            publish_presence(&ctx, false),
            None,
            "a detach that reached the consumer supersedes the pending \
             `true`; republishing that `true` reports an interface that is \
             gone as present"
        );

        // The mirror. The `false` just sent is the squatter now, so the next
        // edge is refused in turn.
        assert_eq!(
            publish_presence(&ctx, false),
            Some(false),
            "a refused edge stays owed in this direction too"
        );
        presence_rx.recv().await.expect("the detach drains");
        assert_eq!(
            publish_presence(&ctx, true),
            None,
            "a bind that reached the consumer supersedes the pending \
             `false`; republishing that `false` reports a live transport as \
             absent"
        );
    }
