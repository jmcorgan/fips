...
pub struct Node {
    ...
    /// `transport::ble::android_io` itself is compiled under, so the seam is
    ...
    ble_radio: Option<Arc<crate::transport::ble::android_io::BleRadioSlot>>,
...
impl Node {
        ...
        if let Some(slot) = self.ble_radio.clone() {
            ...
            for (name, ble_config) in ble_instances {
                ...
                let mut ble = crate::transport::ble::BleTransport::new(
                    ...
                    name,
                    ble_config,
                    crate::transport::ble::android_io::AndroidIo::new(Arc::clone(&slot)),
                    packet_tx.clone(),
                );
                ble.set_local_pubkey(self.identity().pubkey().serialize());
                transports.push(TransportHandle::Ble(ble));
    ...
    ///
    /// ```no_run
    /// # async fn f(node: &mut fips::Node, radio: std::sync::Arc<dyn fips::transport::ble::android_io::AndroidRadio>)
    ...
    /// // time it restarts:
    /// slot.install(fips::transport::ble::android_io::AndroidBleBridge::new(radio));
    /// # Ok(())
    /// # }
    /// ```
    ///
    ...
    pub fn enable_app_owned_ble_radio(
        &mut self,
    ) -> Arc<crate::transport::ble::android_io::BleRadioSlot> {
        Arc::clone(self.ble_radio.get_or_insert_with(|| {
            Arc::new(crate::transport::ble::android_io::BleRadioSlot::new())
        }))
    }
