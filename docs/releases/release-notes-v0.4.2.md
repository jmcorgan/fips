...
**Released**: 2026-08-17 (provisional: the date is confirmed when the
release is tagged)

v0.4.2 is a maintenance release on the v0.4.x line, and the largest one
this line has carried: 117 commits since v0.4.1. Most of it is security
...
another node, or steer that node's path MTU; it fixes NAT traversal in
two places where it was simply not working; and it protects private key
material on disk and clears it in memory. There is no wire format
change.
...
- Two configuration shapes now fail to load: a `node.rekey` interval
  ...
  large for the replay window. Both are start-time failures.
...
- New optional keys: `node.rate_limit.session_setup_burst` /
  `_rate`, `node.rate_limit.established_handshake_burst` / `_rate`, and
  `node.discovery.nostr.max_concurrent_offers_per_npub`.
